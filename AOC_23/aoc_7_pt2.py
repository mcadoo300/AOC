...
def Check_five_kind(cards_dealt, jokers):
    for card in cards:
        if card != 'J' and cards_dealt.count(card) + jokers >=5:
            return True
    return False

def Check_four_kind(cards_dealt,jokers):
    for card in cards:
        if card != 'J' and cards_dealt.count(card) + jokers >=4:
            return True
    return False

def Check_three_kind(cards_dealt,jokers):
    for card in cards:
        if card != 'J' and cards_dealt.count(card) + jokers >=3:
            return True
    return False

def Check_two_kind(cards_dealt, jokers):
    for card in cards:
        if card != 'J' and  cards_dealt.count(card) + jokers >=2:
            return True
    return False
...
def Check_two_pair(cards_dealt, jokers):
    pair = 0
    for card in cards:
        if card != 'J' and cards_dealt.count(card) ==2:
            pair += 1
        elif card != 'J' and  cards_dealt.count(card) + jokers >=2:
            jokers = (cards_dealt.count(card) + jokers)-2
            pair += 1
        if pair == 2:
            return True
    return False
...
def Check_fullhouse(cards_dealt,jokers):
    if jokers > 0:
        pair = 0
        for card in cards:
...
for hand in hands_dealt:
    joker_count = Count_Jokers(hand[0])
    if Check_five_kind(hand[0],joker_count):
        hand.append(five_kind)
    elif Check_four_kind(hand[0],joker_count):
        hand.append(four_kind)
    else:
        if Check_fullhouse(hand[0],joker_count):
            hand.append(full_house)
        elif Check_three_kind(hand[0],joker_count):
            hand.append(three_kind)
        elif Check_two_kind(hand[0],joker_count):
            if Check_two_pair(hand[0],joker_count):
